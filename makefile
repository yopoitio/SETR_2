...
SIMULATOR_NAME = SENSORS
SIMULATOR_FILE_NAME = sensors
...
MAIN_FILES = $(SRC_FOLDER)/main.c $(SRC_FOLDER)/$(MODULE_NAME)/$(FILE_NAME).c $(SRC_FOLDER)/$(SIMULATOR_NAME)/$(SIMULATOR_FILE_NAME).c
TEST_FILES = $(UNITY_ROOT)/src/unity.c $(TEST_FOLDER)/test$(MODULE_NAME).c $(SRC_FOLDER)/$(MODULE_NAME)/$(FILE_NAME).c $(SRC_FOLDER)/$(SIMULATOR_NAME)/$(SIMULATOR_FILE_NAME).c
...
clean:
	$(CLEANUP) $(TARGET) $(MAIN_EXEC) $(TEST_EXEC)
